...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    var inrange = [
      {
        label: translate('Low')
        , data: Math.round(stats.lows * 10000 / daysRecords.length) / 100
      }
      , {
        label: translate('In Range')
        , data: Math.round(stats.normal * 10000 / daysRecords.length) / 100
      }
      , {
        label: translate('High')
        , data: Math.round(stats.highs * 10000 / daysRecords.length) / 100
      }
    ];
    $.plot(
