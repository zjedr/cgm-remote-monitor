...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '<br/><br/>' +
    translate('"Range" for RMS and PGS uses your TARGET range.') +
    '</div><br/><br/>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var colors = ['#f88', '#fb0', '#8f8', '#cf0', '#ff8'];
  var tablecolors = {
    Low: '#f88'
    , Lowish: '#fb0'
    , Normal: '#8f8'
  ...
  ['Low', 'Lowish', 'Normal', 'Elevated', 'High'].forEach(function(range) {
    result[range] = {};
    var r = result[range];
    r.rangeRecords = glucose_data.filter(function(r) {
      if (range == 'Low') {
        return r.sgv > 0 && r.sgv < options.superLow;
      } else if (range === 'Lowish') {
        if (options.superLow == options.targetLow ) {
	  return 0;
	} else {
          return r.sgv >= options.superLow && r.sgv < options.targetLow;
	}
      } else if (range === 'Normal') {
        return r.sgv >= options.targetLow && r.sgv <= options.targetHigh;
      } else if (range === 'Elevated') {
        if (options.targetHigh == options.superHigh) {
	  return 0;
	} else {
          return r.sgv > options.targetHigh && r.sgv <= options.superHigh;
	}
      } else {
        return r.sgv > options.superHigh;
      }
    });
  ...
  result.Normal.readingspct = (100 - result.Low.readingspct - result.High.readingspct - result.Lowish.readingspct - result.Elevated.readingspct).toFixed(1);

  ['High', 'Elevated', 'Normal', 'Lowish', 'Low'].forEach(function(range) {
    var tr = $('<tr>');
    var r = result[range];
    ...
    if (range == 'Low') {
      rangeExp = ' (<' + options.superLow + ')';
    }
    if (range == 'Lowish') {
      rangeExp = ' (>=' + options.superLow + ',&nbsp;<' + options.targetLow + ')';
      // rangeLabel = 'Low-ish';
      rangeLabel = 'Below target';
    }
    if (range == 'Normal') {
      rangeExp = ' (>=' + options.targetLow + ',&nbsp;<=' + options.targetHigh + ')';
    }
    if (range == 'Elevated') {
      rangeExp = ' (>' + options.targetHigh + ',&nbsp;<=' + options.superHigh + ')';
      rangeLabel = 'Above target';
    }
    if (range == 'High') {
      rangeExp = ' (>' + options.superHigh + ')';
    }
    ...
    if (r.rangeRecords.length > 0) {
      ...
      $('<td class="tdborder">' + r.stddev.toFixed(1) + ' (' + Math.round((r.stddev / r.mean) * 100).toFixed(0) + '%)</td>').appendTo(tr);
      $('<td> </td>').appendTo(tr);
    } else {
    ...
    if ((range === 'Lowish' && options.superLow == options.targetLow)
        || (range === 'Elevated' && options.targetHigh == options.superHigh)) {
      ; // Hide these as they obviously don't want them.
    } else {
      table.append(tr);
    }
  });
  ...
  if (glucose_data.length > 0) {
    ...
    $('<td class="tdborder">' + (Math.round(ss.standard_deviation(localBgs) * 10) / 10).toFixed(1) + ' (' + (Math.round(100*(ss.standard_deviation(localBgs)/ss.mean(localBgs)))).toFixed(0) + '%)</td>').appendTo(tr);
  ...
  for (i = 0; i <= glucose_data.length - 2; i++) {
    ...
    if (entry.bgValue > 10) { // options.targetHigh) {
