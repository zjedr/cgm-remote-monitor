...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  if (glucose_data.length > 0) {
    ...
    $('<td class="tdborder">' + (Math.round(ss.standard_deviation(localBgs) * 10) / 10).toFixed(1) + ' (' + Math.round((ss.standard_deviation(localBgs) / ss.mean(localBgs)) * 100).toFixed(0) + '%)</td>').appendTo(tr);
